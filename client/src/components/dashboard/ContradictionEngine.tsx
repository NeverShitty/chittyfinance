...
export default function ContradictionEngine() {
  ...
  const { data: analysisData, isLoading } = useQuery<ContradictionAnalysis>({
  ...
  // Generate resolution plan mutation
  const generateResolutionPlanMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("POST", "/api/contradictions/resolution-plan");
      if (!response.ok) {
        throw new Error("Failed to generate resolution plan");
      }
      return response.json();
    },
    onSuccess: (data) => {
      toast({
        ...
        description: "A detailed resolution plan has been created and will be available in your tasks.",
      });
      // Create a task for the resolution plan
              ...
              {contradictions.length === 0 ? (
                ...
                <div className="space-y-3">
                  ...
                  <Button 
                    ...
                    onClick={() => generateResolutionPlanMutation.mutate()}
                    disabled={generateResolutionPlanMutation.isPending}
                  >
                    {generateResolutionPlanMutation.isPending ? 'Generating...' : 'Generate Detailed Resolution Plan'}
                  </Button>
                </div>
              )}
            </div>
